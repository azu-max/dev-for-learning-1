...
Phase 1: アプリ基盤構築              ✅ 完了
  ↓
Phase 2: Sentry統合（基本導入）       ✅ 完了
  ↓
Phase 3: AWSデプロイ                  ← 次はここ
  ↓
...
| 成果物 | 実践的なエラーキャプチャ設計、アラート設定 |
| 学習ポイント | ダッシュボード活用、エラーレベル設計、4xx/5xx キャプチャ、パフォーマンスモニタリング |
| 詳細 | [Phase 5 詳細](./phase-5/README.md) |

**ステップ（予定）:**
- [ ] Sentryダッシュボードの見方（タグフィルタ、Breadcrumbs、Discover）
- [ ] エラーキャプチャの設計（レベル使い分け、送るべきエラーの判断）
- [ ] HTTP 4xx/5xx レスポンスのキャプチャ追加
- [ ] パフォーマンスモニタリング設定
- [ ] アラートルール設定（Slack / メール通知）
