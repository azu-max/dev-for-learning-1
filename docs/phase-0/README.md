...
```
Round 1 (VPC/Subnet):     ✅✅✅     3/3
Round 2 (Elastic IP/IGW):  ✅✅✅     3/3
Round 3 (CloudWatch/Sentry): ✅✅✅   3/3
Round 4 (EC2/Docker):      ✅✅✅✅   4/4
──────────────────────────────────────
合計: 13/13 (100%) 🏆
```
...
| # | 問題 | 正解 |
|---|---|---|
| Q1 | AWSでEC2を使うために最低限必要なネットワーク空間は？ | VPC |
| Q2 | Public SubnetとPrivate Subnetの違いは？ | 外からアクセスできるか |
| Q3 | VPCの中にSubnetはいくつ作れる？ | 複数作れる |
| Q4 | Internet Gatewayの役割は？ | VPCをインターネットに接続する |
| Q5 | Elastic IPをEC2に紐付けずに放置すると？ | 料金が発生する |
| Q6 | Elastic IPがない場合、EC2のIPはいつ変わる？ | EC2を停止・再起動したとき |
| Q7 | フロントエンドのJSエラーを取得できるのは？ | Sentryのみ |
| Q8 | 同じエラーが100回発生したときのSentryの強みは？ | エラーを自動集計・分類 |
| Q9 | フロントエンドエラーがCloudWatchで取れない理由は？ | ブラウザで発生しEC2に届かない |
| Q10 | OS管理が必要なのはEC2/Lambda/Fargateのどれ？ | EC2 |
| Q11 | EC2でもDocker Composeを使う最大のメリットは？ | ローカルと同じ環境を再現できる |
| Q12 | EC2を1台にした最大のデメリットは？ | その1台が落ちたら全停止（SPOF） |
| Q13 | Monitored External Servicesとは？ | ヘルスチェックの監視対象の外部サイト |
